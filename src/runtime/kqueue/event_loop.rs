//! mio/kqueue event loop for macOS.
//!
...
/// Per-worker connection state for kqueue backend.
struct KqueueConnection {
    stream: TcpStream,
    state: ConnState,
    read_buf_idx: usize,
    write_buf_idx: usize,
...
fn worker_loop(
    ...
    let mut connections: Slab<KqueueConnection> = Slab::with_capacity(max_connections);
...
fn accept_connections(
    ...
    poll: &mut Poll,
    connections: &mut Slab<KqueueConnection>,
    ...
    loop {
        match listener.accept() {
            Ok((stream, peer_addr)) => {
                ...
                let conn_id = connections.insert(KqueueConnection {
                    stream,
                    state: ConnState::Reading {
                        buf_idx: read_buf_idx,
                        filled: 0,
                    },
                    read_buf_idx,
                    write_buf_idx,
...
fn handle_readable(
    ...
    let (read_buf_idx, filled) = match conn.state {
...
fn handle_writable(
    ...
    let (write_buf_idx, written, total) = match conn.state {
        ConnState::Writing {
            buf_idx,
            written,
            total,
    ...
    if new_written >= total {
        // Write complete, go back to reading using the read buffer
        conn.state = ConnState::Reading {
            buf_idx: read_buf_idx,
            filled: 0,
        };
